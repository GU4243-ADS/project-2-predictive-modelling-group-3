...
##1.  HOG features
The random forest model using HOG features performed the best in the testing data with an accuracy rate of 74.8%. 
```{r}
...
system.time(rf <- randomForest(as.factor(y.train) ~ ., 
                   data = training, ntree = 700)) # 7.966 seconds elapsed 

system.time(pred_testing <- test(rf, testing))
```
...
```{r}
# best model
1 - min(errors) ## best model uses 600 trees with a predication accuracy rate of 0.74. 
system.time(rf <- randomForest(as.factor(y.train) ~ ., 
                   data = training, ntree = 200)) # 5.206 seconds

system.time(pred_testing <- test(rf, testing)) # 0.034 seconds to predict
```
...
```{r}
# best model
1 - min(errors) ## best model uses 100 trees with a predication accuracy rate of 0.664. 
system.time(rf <- randomForest(as.factor(y.train) ~ ., 
                   data = training, ntree = 100)) # 1.652 seconds elapsed

system.time(pred_testing <- test(rf, testing)) # 0.014
```
...
```{r}
# best model
1 - min(errors) ## best model uses 200 trees with a predication accuracy rate of 0.768. 
system.time(rf <- randomForest(as.factor(y.train) ~ ., 
                   data = training, ntree = 200)) # 4.700 seconds elapsed
...
```{r}
# best model
1 - min(errors) ## best model uses 200 trees with a predication accuracy rate of 0.74. 
system.time(rf <- randomForest(as.factor(y.train) ~ ., 
                   data = training, ntree = 200)) # 5.539 seconds elapsed

system.time(pred_testing <- test(rf, testing)) # 0.041
```


## Combined SIFT & HOG & LBP & Color
```{r}
# Training/Testing 
