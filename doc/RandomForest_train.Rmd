...
## Cross Validation
Based on code by Yuting Ma (Spring 2016)
```{r}
m <- length(y.train)
m.fold <- floor(m/K)
...
# Get the lowest error rate of cv
...
```{r}
#system.time -- to find time it takes to run
rf <- tuneRF(x.train[s != best,], as.factor(y.train[s != best]), ntree = 600, doBest = TRUE)
